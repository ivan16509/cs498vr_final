...
public class GravityPlayer2 : MonoBehaviour
{
    /// <summary>
    ...
    protected virtual void UpdateController()
    {
        ...
        if (IsGrounded() && MoveThrottle.y <= transform.lossyScale.y * 0.001f)
        {
            ...
            moveDirection -= bumpUpOffset * Vector3.up * gravDir;
        }
    ...
    {   
        ...
        if (EnableLinearMovement)
        {
            ...
            if (primaryAxis.x < 0.0f)
                MoveThrottle += ort * (Mathf.Abs(primaryAxis.x) * transform.lossyScale.x * moveInfluence *
                                       BackAndSideDampen * Vector3.left *gravDir );

            if (primaryAxis.x > 0.0f)
                MoveThrottle += ort * (primaryAxis.x * transform.lossyScale.x * moveInfluence * BackAndSideDampen *
                                       Vector3.right*gravDir);
        }
        ...
        if (EnableRotation)
        {
            ...
            else
            {
                ...
                euler.y += secondaryAxis.x * rotateInfluence * gravDir;
            }
